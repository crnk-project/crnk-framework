...
By default, the naming convention for `@JsonApiRelationId` field is to end with a `Id` suffix. Crnk will the
pair those two objects automatically. But it is also possible to specify a custom name, for example:
