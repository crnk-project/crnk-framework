...
A relationship backed by a primitive relationship identifier `projectId` and matching relationship `project`
where the identifier is always written in the response with `SerializeType.ONLY_ID` and the relationship
is looked up in the project repository when necessary with `LookupIncludeBehavior.AUTOMATICALLY_WHEN_NULL`:
...
can be implemented with custom logic to lookup relationships.
