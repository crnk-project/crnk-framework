...
There is an `ActivitiModule` for the https://www.activiti.org[Activiti] workflow engine that offers an alternative REST.
...
- have a type-safe, non-generic REST API that is tailored towards the use cases at hand. This means that for each process
  ...
  both the static fields provided by Activiti (like `name`, `startTime` and `priority`) and the dynamic variables
  stored by the application. Mapping to static resp. dynamic fields is done automatically by the `ActivitiModule`
  and hidden from consumers. The repository implementations ensure a proper isolation of different types. And
  the application is enabled, for example, to introduce custom security policies for each resource with
  the `SecurityModule` or otherwise.

This setup is in stark contrast to the API provided by Activiti that is fully generic.
...
The `ActivitiModule` comes with four resource base classes that match the equivalent Activiti classes:
...
- `scheduleChangeApproved` invokes `${approvalManager.approved(execution)}` whereas `approvalManager` is a Java object
  ...
  changes. Real world use cases may also need to save and reconstruct the security context.
...
`ApprovalIntTest` showcases the entire example application and `ActivitiModule` as test cases doing a change,
