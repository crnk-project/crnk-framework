...
resource starts holding complex object graphs again. For this reasons the `OperationsModule` provides support for
http://jsonpatch.com/[jsonpatch.com]. It allows to send multiple insertions, updates and deletions with a single request
and provides the results for each such executed operation.
