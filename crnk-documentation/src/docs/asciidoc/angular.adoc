...
Tables display JSON API resources and allow sorting, filtering, paging.
Forms display JSON API resources and allow to trigger `POST`, `PATCH` and `DELETE` requests.
In case of errors, the errors get displayed to the user, either in a header section or next to the
input component in question (determined by the user of JSON API source pointers). For this purpose
crnk provides two tools: `crnk-gen-typescript` and `@crnk/angular-ngrx`. `crnk-gen-typescript`
generates type-safe Typescript stubs for a backend application. `@crnk/angular-ngrx` takes care
...
`@crnk/angular-ngrx/expression` provides a QueryDSL-like expression model for Typescript. It is used  to
 ...
 - Errors that cannot be mapped to a `FormControl` should be displayed in a editor header or error dialog.

`ngModel` is limited to holding a simple value, but the use cases here require the full resource and the path to the
field to determine the field value and errors. This is achieved in `@crnk/angular-ngrx/expression` with:
