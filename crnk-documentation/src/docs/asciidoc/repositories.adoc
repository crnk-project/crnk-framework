...
INFO: Relationships that are backed by a further <<jsonApiRelationId,`@JsonApiRelationId`>> field
do not have to implement a relationship repository. If not, Crnk will automatically provide a
default implementation that dispatches requests to the owning and opposite `ResourceRepository`, which
in turn allows to focus the development to resource repositories.
