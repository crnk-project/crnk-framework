...
is implemented by <<urlmapper,`QuerySpecUrlMapper`>>.
...
Sorting parameters are reprsented by `SortSpec` within `QuerySpec` similar to `FilterSpec` above.

anchor:pagination[]

### Pagination

Resources can specify pagination with <<jsonApiResourcePagingBehavior,@JsonApiResource.pagingBehavior>>.
...
Paging parameters are represented by `PagingSpec` within `QuerySpec`.

By default, repositories can access the parameter with `QuerySpec.getLimit` and `QuerySpec.getOffset`.
In case of custom strategies, `QuerySpec.getPagingSpec` must be used.
...
Note that a QuerySpec instance holds the parameters for a single resource type.
In case of inclusions, the QuerySpec references further
QuerySpecs, one for each inclusion holding the parameters of that inclusion.
Repositories are then accessed with the QuerySpec matching the repository.
The QuerySpec for a particular type can be obtained with `QuerySpec.getRelatedSpec(Class)` on the root QuerySpec.
