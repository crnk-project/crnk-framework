...
defined implementing `RelationshipRepository`. `RelationshipRepository` implements the methods
...
### BulkRelationshipRepository

BulkRelationshipRepository extends RelationshipRepository and provides an additional
...
Note that in contrast to `ResourceRepository` and `RelationshipRepository` that are symmetric
and applicable to Crnk servers and client, `BulkRelationshipRepository` is applicable on the server-side only.
