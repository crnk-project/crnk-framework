...
- `TypeParser` allows to register and override the parsing of Strings to other classes.
  By default it has has built-in parsers for various JRE classes, it looks on the type
  itself for a String constructor or static `parse(String)` parse method and otherwise
  fallbacks to Jackson. Further `StringParser` can add support for arbitrary types.
