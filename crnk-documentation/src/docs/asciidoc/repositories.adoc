...
implementations. For this reason `crnk-engine` makes use of `QueryAdapter` and allows implementations other than
...
resources. Crnk then fills in PagedLinksInformation with the corresponding links.
...
They should be located in your resource search package. If crnk-cdi or crnk-spring
