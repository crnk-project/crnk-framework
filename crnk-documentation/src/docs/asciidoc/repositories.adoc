...
the operator and the filter value. Internally Crnk will map filter parameters to
properly typed `FilterSpec.value`. The type is determined based on the type of
the filtered attribute. The process can be customized by the `QuerySpecUrlMapper`
and `TypeParser` implementations that are accessible from `CrnkBoot`.
...
Crnk comes by default with offset/limit pagination support. The format for pagination is
then `page[offset|limit]="value", where value is an integer`, for example:

`GET /tasks?page[offset]=0&page[limit]=10`

The parameters are then available with `QuerySpec.getLimit` and `QuerySpec.getOffset`.
...
Internally offset/limit pagination is implemented with `OffsetLimitPagingBehavior` and `OffsetLimitPagingSpec`.
They implement the more generic `PagingBehavior` and `PagingSpec`. An instance of `PagingSpec`
is hold by QuerySpec and is, for example, also accessed by `QuerySpec.getLimit` and `QuerySpec.getOffset`.
`PagingSpec` is directly accessible with `QuerySpec.getPagingSpec` and `QuerySpec.setPagingSpec`.

Applications are free to implement other pagination behaviors implementing `PagingBehavior` and `PagingSpec`.
JSON API does not mandate the use of a specific strategy and lets application choose the best suited mechanism.
In order to do so, you would have to perform the following actions:
...
** by default the new pagination strategy will become the default strategy for all resources.
** multiple pagination strategies can be registered and then selected for each resource individually with
  <<jsonApiResourcePagingBehavior,@JsonApiResource.pagingBehavior>>.

For an example follow the `OffsetLimitPagingBehavior` and `OffsetLimitPagingSpec` implementation. New implementations may
extend from those classes to offer compatibility in two different ways:

- maintain support for `offset`, `limit` parameters.
- translate the custom page parameters to `offset`, `limit` to make existing (potentially third-party) repositories compatible with the custom parameters.
...
* `setEnforceDotPathSeparator(boolean)`
	DefaultQuerySpecUrlMapper by default supports by default two URL conventions:
	 ...
	 to be enabled and will become the default at some point in the future. Note that without the enforcement,
...
----
include::../../../../crnk-activiti/src/test/java/io/crnk/activiti/example/approval/ApprovalRepositoryDecorator.java[tags=docs_decorator]
----
...
.ApprovalRepositoryDecorator.java
----
include::../../../../crnk-activiti/src/test/java/io/crnk/activiti/example/approval/ApprovalRepositoryDecorator.java[tags=docs_save]
----
