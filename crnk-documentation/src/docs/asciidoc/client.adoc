...
communicating with JSON-API compliant servers. Three http client libraries are supported:
...
* `RestTemplate` from Spring that provides a Spring abstraction of other HTTP libraries.
  ...
  For some more setup information, have a look at the subsequent sections.
