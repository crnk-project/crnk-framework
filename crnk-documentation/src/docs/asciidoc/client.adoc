...
Crnk clienet and server share the URL handling. `QuerySpecUrlMapper` performs the mapping of HTTP request parameters to QuerySpec.
...
`HttpAdapter`. Both implementations provide a
...
- https://github.com/crnk-project/crnk-framework/blob/master/crnk-monitor-brave4/src/main/java/io/crnk/monitor/brave/BraveClientModule.java[crnk-monitor-brave4] for an advanced example.
