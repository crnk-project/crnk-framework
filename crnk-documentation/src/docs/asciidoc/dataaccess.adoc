...
category. Those categories, denoted as facets, have a label, a set of values, a count for each value. Facets
may also depend on each other, whereas later facets are filtered by former ones.

With `FacetModule` Crnk provides an out-of-the-box solution to implemented faceted search.
There is an inefficient in-memory implementation that works well with every repository as long as the
data set is small. And later more efficient implementations tailored towards the various repository
implementations.

### Setup

The `FacetModule` can
be added to the application. For Spring there is a `CrnkFacetAutoConfiguration` in place.
All that is needed is to annotate the desired attributes to be faceted:
...
Behind the scenes are currently two implementations available that are automatically chosen for the repository at hand:
...
A facet value is selected through its label and the special `SELECT` operator. In the given example, the priority
is restricted to `1` and `2`. This in turn can limit the counts other facets, if those facets follow
