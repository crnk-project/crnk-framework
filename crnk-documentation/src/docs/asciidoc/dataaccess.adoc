...
It is possible to add additional JSON:API related fields to entities by annotating them with `@jakarta.persistence.Transient` (or the other way around by marking
it with `@JsonIgnore`).
...
resource validation and provides exception mappers for `jakarta.validation.ValidationException` and `jakarta.validation.ConstraintViolationException`.
Among others, it properly translates 'jakarta.validation.ConstraintViolation' instances to JSON:API errors.
