...
1. `JsonAnyGetter` and `JsonAnySetter` annotations from Jackson allow to dynamically add properties to a resource.
  Further implementing `ResourceTypeHolder` interface allows to also customize the type of a resource. Together it
   allows the implementation of arbitrarily custom resources. Note that when manipulating with `ResourceTypeHolder`
   it may also become necessary to register the resource with the `ResourceRegistry` as outlined in the
   similar `Resourrce`example below.
2. Create a matching resource class at runtime with a library like http://bytebuddy.net/#/ to follow the same
